...
// --- Output generation -------------------------------------------
function generateData(listId, domains, pathRules, exceptions) {
...
async function main() {
  ...
  for (const id of ids) {
    ...
    try {
      ...
      if (!dryRun) {
        const data = generateData(id, domains, pathRules, exceptions);
        const outPath = path.join(outputDir, "external", id + ".json");
        const { written } = writeIfChanged(outPath, data);
        const sizeMb = (Buffer.byteLength(JSON.stringify(data, null, 0)) / 1024 / 1024).toFixed(1);
        console.log("  → " + outPath + " (" + sizeMb + " MB)" + (written ? "" : " [unchanged]"));
      }
    } catch (err) {
