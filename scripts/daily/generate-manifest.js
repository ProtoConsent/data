...
// --- Read region codes from config/regional-languages.json ---
const REGIONAL_LANGUAGES_PATH = path.join(__dirname, "..", "..", "config", "regional-languages.json");
let RL_DATA;
try {
  RL_DATA = JSON.parse(fs.readFileSync(REGIONAL_LANGUAGES_PATH, "utf-8"));
  if (!RL_DATA || typeof RL_DATA !== "object" || Array.isArray(RL_DATA)) {
    throw new Error("expected a JSON object with region codes as keys");
  }
  if (Object.keys(RL_DATA).length === 0) {
...
const LIST_CATALOG = {
  ...
  // --- Regional aggregate entries (backward compat for old extension <=0.7.x) ---
  regional_cosmetic: {
    ...
    preset: "basic",
    order: 21,
    regions: Object.keys(RL_DATA).sort(),
...
// --- Aggregate metadata from all regional merged files (backward compat) ---
function readRegionalAggregate(enhancedDir, suffix) {
  ...
  const regions = Object.keys(RL_DATA).sort();
...
function buildManifest(enhancedDir) {
  ...
  // Per-source regional entries are NOT included in the CDN catalog.
  // Old extensions (<=0.7.x) would download them as regular lists since they
  // lack the region filter. The new extension merges per-source entries from
  // its bundled catalog instead. Re-enable once old versions are gone from stores.
