...
// Load regional config for slug mappings
const RL_CONFIG_PATH = path.join(__dirname, "..", "..", "config", "regional-languages.json");
const RL_CONFIG = JSON.parse(fs.readFileSync(RL_CONFIG_PATH, "utf-8"));

// URL index groupings for regions where config.sources.length != REGIONS.sources.length
// Each sub-array lists the URL indices that belong to one slug.
const URL_GROUPS = {
  ru: [[0], [1, 2]],
};
...
async function main() {
  ...
  for (const regionId of regionIds) {
    ...
    if (!dryRun) {
      // --- Per-source output (new architecture) ---
      const configEntry = RL_CONFIG[regionId];
      if (configEntry && configEntry.sources) {
        const groups = URL_GROUPS[regionId] || configEntry.sources.map((_, i) => [i]);
        for (let gi = 0; gi < groups.length; gi++) {
          const slug = configEntry.sources[gi].slug;
          if (!slug) continue;
          const indices = groups[gi];
          const groupBlocking = indices.map(i => blockingResults[i]).filter(Boolean);
          const groupCosmetic = indices.map(i => cosmeticResults[i]).filter(Boolean);
          if (groupBlocking.length === 0) continue;

          const bMerged = mergeBlocking(groupBlocking);
          const cMerged = mergeCosmetic(groupCosmetic);

          const bId = "regional_" + regionId + "_" + slug + "_blocking";
          const cId = "regional_" + regionId + "_" + slug + "_cosmetic";
          const bData = generateBlockingJson(bId, bMerged.domains, bMerged.pathRules);
          const cData = generateCosmeticJson(cId, cMerged.generic, cMerged.domains, cMerged.exceptions);

          const bFile = path.join(outputDir, bId + ".json");
          const cFile = path.join(outputDir, cId + ".json");
          writeIfChanged(bFile, bData);
          writeIfChanged(cFile, cData);

          // Write back metadata to config for CDN version info.
          // Read the actual file (writeIfChanged may have skipped it if content
          // was unchanged, preserving the old version/generated timestamps).
          const actualB = JSON.parse(fs.readFileSync(bFile, "utf-8"));
          const actualC = JSON.parse(fs.readFileSync(cFile, "utf-8"));
          const src = configEntry.sources[gi];
          src.blocking_version = actualB.version;
          src.blocking_generated = actualB.generated;
          src.blocking_domains = actualB.domain_count;
          src.blocking_paths = actualB.path_rule_count;
          src.cosmetic_version = actualC.version;
          src.cosmetic_generated = actualC.generated;
          src.cosmetic_generic = actualC.generic_count;
          src.cosmetic_domain_rules = actualC.domain_rule_count;
        }
      }

      // --- Merged output (backward compat for old extension <=0.7.x) ---
      const blockingId = "regional_" + regionId + "_blocking";
      const cosmeticId = "regional_" + regionId + "_cosmetic";
      const blockingData = generateBlockingJson(blockingId, blocking.domains, blocking.pathRules);
      const cosmeticData = generateCosmeticJson(cosmeticId, cosmetic.generic, cosmetic.domains, cosmetic.exceptions);
      const blockingPath = path.join(outputDir, blockingId + ".json");
      const cosmeticPath = path.join(outputDir, cosmeticId + ".json");
      writeIfChanged(blockingPath, blockingData, { force: true });
      writeIfChanged(cosmeticPath, cosmeticData, { force: true });
    }
  }

  // Write updated regional-languages.json with version/counts metadata
  if (!dryRun) {
    fs.writeFileSync(RL_CONFIG_PATH, JSON.stringify(RL_CONFIG, null, 2) + "\n", "utf-8");
    console.log("\n  -> " + RL_CONFIG_PATH + " (metadata updated)");
  }
