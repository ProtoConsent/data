...
async function main() {
  ...
  const { written: egWritten } = writeIfChanged(egPath, enhancedGlobal, { indent: 2, suffix: "\n" });
  const { written: esWritten } = writeIfChanged(esPath, enhancedSites, { indent: 2, suffix: "\n" });

  console.log("\nWritten (enhanced lists):");
  console.log("  " + egPath + (egWritten ? "" : " [unchanged]"));
  console.log("  " + esPath + (esWritten ? "" : " [unchanged]"));
