...
function isUnchanged(jsonPath, sortedDomains, sortedPaths) {
  try {
    const existing = JSON.parse(fs.readFileSync(jsonPath, "utf-8"));
    return arraysEqual(existing.domains, sortedDomains)
        && arraysEqual(existing.paths || [], sortedPaths);
  } catch (_) {
    return false;
  }
}
...
function writePurposeFiles(purpose, label, description, version, now, sortedDomains, sortedPaths, suffix) {
  const jsonPath = path.join(OUT_DIR, "json", `protoconsent_${purpose}${suffix}.json`);
  if (isUnchanged(jsonPath, sortedDomains, sortedPaths)) {
    return false;
  }
...
function writeProfileFiles(profileId, label, description, version, now, purposes, sortedDomains, sortedPaths, suffix, abpModifier, adguardModifier, abpDomainModifiers, adguardDomainModifiers) {
  const jsonPath = path.join(OUT_DIR, "json", `protoconsent_${profileId}${suffix}.json`);
  if (isUnchanged(jsonPath, sortedDomains, sortedPaths)) {
    return false;
  }
...
function main() {
  ...
  for (const purpose of PURPOSES) {
    ...
    const lightWritten = writePurposeFiles(purpose, label, PURPOSE_DESCRIPTIONS[purpose], purposeVersion, now, lightDomains, lightPaths, "");

    // Write extended (_extended suffix) = bundle + delta
    const extWritten = writePurposeFiles(purpose, label, PURPOSE_DESCRIPTIONS[purpose], purposeVersion, now, extDomains, extPaths, "_extended");

    const status = (lightWritten === false && extWritten === false) ? " [unchanged]" : "";
    console.log(`  ${purpose}: ${lightDomains.length} light + ${extDomains.length} extended, ${extPaths.length} paths` + (status || " -> json | hosts | domains | abp | adguard"));
  }
  ...
  for (const [profileId, profile] of Object.entries(PROFILES)) {
    ...
    const pLightWritten = writeProfileFiles(profileId, label, profile.description, version, now, profile.purposes, lightDomains, lightPaths, "", profile.abpModifier, profile.adguardModifier, lightAbpMods, lightAdgMods);
    const pExtWritten = writeProfileFiles(profileId, label, profile.description, version, now, profile.purposes, extDomains, extPaths, "_extended", profile.abpModifier, profile.adguardModifier, extAbpMods, extAdgMods);

    const pStatus = (pLightWritten === false && pExtWritten === false) ? " [unchanged]" : "";
    console.log(`  ${profileId}: ${lightDomains.length} light + ${extDomains.length} extended, ${extPaths.length} paths` + (pStatus || ` -> json | hosts | domains | abp | adguard (${profile.purposes.join(" + ")})`));
  }
