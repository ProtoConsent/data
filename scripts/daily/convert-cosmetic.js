...
async function main() {
  ...
  if (dryRun) {
    ...
    const outPath = path.join(outputDir, "external", "easylist_cosmetic.json");
    const { written } = writeIfChanged(outPath, output);
    const sizeKb = (Buffer.byteLength(JSON.stringify(output)) / 1024).toFixed(1);
    console.log("  -> " + outPath + " (" + sizeKb + " KB)" + (written ? "" : " [unchanged]"));
  }
}
