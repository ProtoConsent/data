...
async function main() {
  ...
  if (dryRun) {
    ...
    var cookiePath = path.join(outputDir, "external", "easylist_cookie_cosmetic.json");
    var { written: cookieWritten } = writeIfChanged(cookiePath, cookieOutput);
    console.log("\n  -> " + cookiePath + (cookieWritten ? "" : " [unchanged]"));

    var waPath = path.join(outputDir, "external", "webannoyances_cosmetic.json");
    var { written: waWritten } = writeIfChanged(waPath, waOutput);
    console.log("  -> " + waPath + (waWritten ? "" : " [unchanged]"));
  }
}
