...
async function main() {
  ...
  if (dryRun) {
    ...
    const { written: sigWritten } = writeIfChanged(sigPath, sigData, { indent: 2, suffix: "\n" });
    console.log("\n  -> " + sigPath + (sigWritten ? "" : " [unchanged]"));

    const sitePath = path.join(autoconsentOutputDir, SITE_FILE);
    const { written: siteWritten } = writeIfChanged(sitePath, siteSpecificOut, { indent: 2, suffix: "\n" });
    console.log("  -> " + sitePath + (siteWritten ? "" : " [unchanged]"));

    const detectorsPath = path.join(autoconsentOutputDir, DETECTORS_FILE);
    const { written: detWritten } = writeIfChanged(detectorsPath, detectorsOut, { indent: 2, suffix: "\n" });
    console.log("  -> " + detectorsPath + (detWritten ? "" : " [unchanged]"));
  }
}
